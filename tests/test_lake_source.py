...
- # G1-VERIFY r_event 因子推导逻辑自洽（临时库构造已知除权事件，af1=af3×F 精确验证）
...
def _insert_kline(con, ts_code, closes_by_date, volume=1000, is_st_t2=0):
...
def test_noop_methods(tmp_path):
    ...
    # adjfactor_fetch 返回推导事件（非 no-op，报告 A#13）
    ...
    assert [r[1] for r in rows] == ["2026-07-10"]
...
# --------------------------------------------------------------------------- G1-VERIFY r_event
def test_r_event_factor_derivation_selfconsistent(tmp_path):
    """# G1-VERIFY 逻辑自洽：构造已知除权事件，验证 F 累乘与 af1=af3×F 精确成立。

    造数（sh.600001，平盘 + 两个除权日跳空，跳幅 <30% 在 sanity cap 内）：
    - ex1=2026-07-10(周五)：前收(07-09)=10 → 当日 close 8   ⇒ r1 = 10/8 = 1.25，F=1.25
    - ex2=2026-08-10(周一)：前收(08-07)=8 → 当日 close 7    ⇒ r2 = 8/7 ≈ 1.142857，F≈1.428571
    正确后复权序列应恒为 10（af3×F 抵消跳空）——与 BaoStock hfq 语义同构。
    """
    db = _make_lake(tmp_path)
    con = duckdb.connect(db)
    _insert_master(con, "sh.600001", "测试银行")
    ...
    for d in days:
        if d < date(2026, 7, 10):
            ...
            closes[d] = 7.0
    # 精确控制前收：ex1 前一日=10、ex2 前一日=8（跳空只在 ex_date 当日发生）
    closes[date(2026, 7, 9)] = 10.0
    closes[date(2026, 8, 7)] = 8.0
    _insert_kline(con, "sh.600001", closes)
    ...
    f = _fetcher(db)
    af = f.adjfactor_history("sh.600001")
    assert af is not None
    assert [(r[1], r[3]) for r in af["rows"]] == [
        ("2026-07-10", "1.250000"), ("2026-08-10", f"{1.25 * (8 / 7):.6f}")]

    rebuilt = f.kline_af3_rebuilt("sh.600001")
    assert rebuilt is not None and len(rebuilt["dates"]) == len(days)
    # af1 恒=10（af3×F 精确抵消）——后复权序列连续性的定义性检验
    for d, a3, a1 in zip(rebuilt["dates"], rebuilt["af3_close"], rebuilt["af1_close"]):
        assert a1 == pytest.approx(10.0), f"{d}: af3={a3} af1={a1}"


def test_r_event_sanity_cap_skips_anomaly(tmp_path):
    """|r_event-1| > factor_sanity_cap_pct（30%）→ 跳过事件+不污染序列。"""
    db = _make_lake(tmp_path)
    con = duckdb.connect(db)
    _insert_master(con, "sh.600001", "测试银行")
    days = list(_weekdays(date(2026, 7, 1), RUN_DAY))
    closes = {d: 10.0 for d in days}
    closes[date(2026, 7, 9)] = 10.0
    closes[date(2026, 7, 10)] = 5.0    # r = 10/5 = 2.0 → 超 ±30% sanity
    _insert_kline(con, "sh.600001", closes)
    _insert_dividend(con, "sh.600001", date(2026, 7, 10), 0.5)
    _insert_t7(con, date(2026, 7, 1), RUN_DAY)
    con.close()
    f = _fetcher(db)
    assert f.adjfactor_history("sh.600001") is None   # 唯一事件被 sanity 剔除


def test_no_dividends_no_factors(tmp_path):
    f = _fetcher(_base_lake(tmp_path))
    assert f.adjfactor_history("sz.000003") is None
    rebuilt = f.kline_af3_rebuilt("sz.000003")
    # 无除权事件 → F≡1 → af1 == af3
    for a3, a1 in zip(rebuilt["af3_close"], rebuilt["af1_close"]):
