...
| **v6.2.1-D1** | **409 重名 toast 文案修复** | 前端读 `e.body.detail.error`（FastAPI HTTPException detail=dict 标准包裹；detail 为对象时才取 .error），非重名 409 走通用 toast；测试 mock 对齐真实响应形状 + 补回归用例（vitest 214→215） |
