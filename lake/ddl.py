...
DDL_STATEMENTS = [
  ...
  source VARCHAR, fetched_at TIMESTAMP, data_version VARCHAR, PRIMARY KEY(ts_code, date))""",
    # T3
  ...
  source VARCHAR, fetched_at TIMESTAMP, data_version VARCHAR)""",   # cash_dps=元/股(em dps_pretax 已/10)；stk_div 暂无源→NULL
    # T5 PIT：pub_date 必存
  ...
  source VARCHAR, fetched_at TIMESTAMP, data_version VARCHAR, PRIMARY KEY(index_code, date))""",
